...
async def check_subscription(user_id: int, bot) -> bool:
    ...
    for channel in REQUIRED_CHANNELS:
        try:
            ...
            # restricted - подписан но с ограничениями
            ...
            print(f"⚠️ БЛОКИРУЕМ доступ из-за невозможности проверить подписку")
            # ВАЖНО: Возвращаем FALSE чтобы бот НЕ работал если не можем проверить!
            # Это обеспечит строгую защиту
            return False
    return True
...
async def show_subscription_request(update: Update, lang: str):
    ...
    # Добавляем кнопку проверки ВНИЗУ
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Только кнопка проверки подписки разрешена для неподписанных
    if query.data != 'check_sub':
        is_subscribed = await check_subscription(user_id, query.bot)
        if not is_subscribed:
            await query.answer(
                '🔒 Подпишись на канал @Mollysantana_Killaz!\n\n'
                'Используй /start',
                show_alert=True
            )
            return
